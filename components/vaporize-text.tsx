...
export default function VaporizeText({
  ...
  const handleMouseEnter = useCallback(() => {
    if (animationState !== "static") return; // guard against re-trigger mid-play
    vaporizeProgressRef.current = 0;
    resetParticles(particlesRef.current);
    setAnimationState("vaporizing");
  }, [animationState]);

  // ── Animation loop — runs only while vaporizing / fadingIn ─
  useEffect(() => {
      ...
      if (animationState === "vaporizing") {
        ...
        if (textBoundaries) {
          ...
          if (vaporizeProgressRef.current >= 100 && allVaporized) {
            // Re-create particles for the same text so fade-in renders fresh
            renderCanvas({
              text,
              font,
              color,
        ...
        if (fadeOpacityRef.current >= 1) {
          // Done — go back to static, ready for next hover
          resetParticles(particlesRef.current);
          memoizedRenderParticles(ctx, particlesRef.current);
  ...
  // ── Render canvas when size / font changes ─────────────────
  useEffect(() => {
    renderCanvas({
      text,
      font,
  ...
  }, [text, font, color, alignment, wrapperSize, globalDpr, transformedDensity]);
  ...
  return (
    ...
    >
      {/* Hidden text for sizing + SEO + accessibility */}
      <span
        style={{
          visibility: "hidden",
          whiteSpace: "nowrap",
...
const renderCanvas = ({
  ...
  const color = parseColor(rawColor ?? "rgb(255, 255, 255)");
...
const createParticles = (
  ...
  if ("fontKerning" in ctx) {
    (ctx as unknown as Record<string, string>).fontKerning = "normal";
  }
  if ("textRendering" in ctx) {
    (ctx as unknown as Record<string, string>).textRendering =
      "geometricPrecision";
  }
...
const resetParticles = (particles: Particle[]) => {
  particles.forEach((particle) => {
    ...
    particle.shouldFadeQuickly = undefined;
  });
};
